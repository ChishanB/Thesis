...
# Version notes: 
## v.0.4: Handle both hs (ENSG) and ms (ENSM) input.
## Keeps .ensemblID period, to ensure matching of IDs with covnersion table. However, the script can handle both formats. It automatically adjusts the gene annotation file if it contains .version so that it can match those without.
## Allows removable of specified file prefix and suffix. If processing via NCI pipeline. This is unlikely to need changing....
## v.0.3: This has been updated to ensure the output table doesn't contain the ensmemblID.version (only ensemblID with no period).
...
##############################
# Import merged data
merged_data <- read.delim("./1_data/150bp/raw_counts/TE_and_gene_counts_Updated_and_Raw.txt")

select(merged_data,-Name)
select(merged_data,-Family)
select(merged_data,-Class)
select(merged_data,-Family_Class)

metadata <- read.delim("./1_data/150bp/raw_counts/TE_only_and_all_counts.txt")

table %>%
  full_join(merged_data, metadata)
...
update_table <- function(data, 
                         anno_file = "./1_data/150bp/seans_scripts/annos/genes/gencode.vM25.gene_annotation_table.txt", 
                         output.dir = "./3_results",
                         output.appendix = "",
                         genome = "hs", # Determines which prefix to search genes for 'hs'(ENSG) 'ms' (ENSM)
                         remove.prefix = "2_output/align/", # Remove filename prefix (given by snakemake TEtranscripts pipeline)
                         remove.extension = ".bam" # Remove filename extension, typically '.bam'
                         ){
  ...
  if(genome == "hs") {
    ...
    Number_of_Genes <- sum(grepl("ENSM",data$gene.TE)) # Since ensembl IDs are used by TEtranscripts, and no TEs use this nomenclature one can count how many rows start with 'ENSG' to know how many genes are present.
  }
  ...
  ## TEtranscripts always outputs genes first then TEs, so it's safe to repeat the number of genes first, then TEs.
  data <- data %>% mutate(Type = c(rep("gene", Number_of_Genes), rep("TE", Number_of_TEs)) ) 
  nrow(data)

  # Retain only sample ID and remove file extension characters
  cat("\nRemoving filename prefix: ", remove.prefix, "\nand extension: ", remove.extension)
  data <- data %>%
    rename_with(~ gsub(remove.prefix, "", .x, fixed = TRUE)) %>%
    rename_with(~ gsub(remove.extension, "", .x, fixed = TRUE))
  ...
  ### Remove check for now, as we're using an updated GTF file, for both ms and hs...
  # nrow(data) == 60165 
  ...
  gene_IDs$GeneSymbol <- trimws(gene_IDs$GeneSymbol, which = c("left")) # remove trailing whitespace that might be present in annotation conversion file
  ...
  if (sum(grepl("ENS*\\.*", data$gene.TE)) > 1){
  ...
  } else if(sum(grepl("ENS*\\.*", data$gene.TE)) == 0) { # No ensembl.ID and version
