...
9.  Volcano plots (from step 7/8 DE results) showing those that are
    ...
    plot_1 <- res_dt %>%
      ...
      scale_color_manual(
        values = colours_1,
        labels = c("Downregulated", "Not Significant", "Upregulated") # Adjust to your specific categories
      ) +
      scale_fill_manual(
        values = colours_1,
        labels = c("Downregulated", "Not Significant", "Upregulated") # Adjust to your specific categories
    ...
    plot_3 <- TE_rows %>%
      ...
      scale_fill_manual(
        values = colours_1,
        labels = c("Not Significant", "Upregulated") # Adjust to your specific categories
      ) +
      scale_alpha_manual(
        values = opacity,
        labels = c("Downregulated", "Not Significant", "Upregulated") # Adjust to your specific categories
...
10. MA plots (from step 7/8 DE results) colour coded to mark genes that
    ...
    # MA plots colour coded to mark genes that are FDR<0.05 – 1 x Gene, 1 x TE

    gene_subset <- res[grep("^ENSMUSG\\d+$", rownames(res)), ]
    # Reverse grep to get rows that do NOT match the pattern
    TE_subset <- res[!grepl("^ENSMUSG\\d+$", rownames(res)), ]


    #pdf("gene_MA_plot.pdf") 
    # This function highlights points with FDR or padj < 0.05 in blue.
    plotMA(gene_subset)
    #dev.off()  


    ## WHYYYY AM I GETTING 4 HIGHLIGHTED BLUE POINTS HERE
    plotMA(TE_subset)

    # Filter DESeq2 results to keep only those with padj < 0.05
    significant_results <- TE_subset[!is.na(TE_subset$padj) & TE_subset$padj < 0.05, ]

    ## WHY IS THERE 2 SIGNIFICANT HERE WHEN I ONLY HAD 1 IN MY VOLCANO PLOT!!!
    # View the subset of significant results
