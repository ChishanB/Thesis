...
data <- read.xlsx("./1_data/tRNA_quantification_raw.xlsx")
...
# save that
#write.csv(df_combined, file = "./1_data/miRMaster_raw_formatted/miRmaster_tRNAs_raw.csv", row.names = FALSE)

# Reuse and repeat this for other sncRNA types
# Get rid of NAs in next step when we combine them all and do DESeq2.

lala <- read.csv("./1_data/miRMaster_raw_formatted/miRmaster_miRNAs_raw.csv")
lala <- lala %>%
  rename(Reference = miRNA)
#write.csv(lala, file = "./1_data/miRMaster_raw_formatted/miRmaster_miRNAs_raw.csv", row.names = FALSE)
