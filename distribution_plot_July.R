...
miRNAs$V1 <- gsub(" mature$", "", miRNAs$V1)
...
#[1] "mmu-miR-128-3p"  "mmu-miR-145a-5p" "mmu-miR-199a-3p" "mmu-miR-199b-3p" "mmu-miR-19b-3p"  "mmu-miR-211-5p"  "mmu-miR-221-3p" 
#[8] "mmu-miR-410-3p"  "mmu-miR-532-5p" 

# So 8 of the DE miRNAs had targets in multimiR
...
#write.csv(miRNA_target_numbers, file = "./3_results/july_DE_mature_miRNA_hits_freq.csv")
# > 2231 + 441 + 156 + 149 + 7263 + 175 + 4949 + 473 + 630
#[1] 16467
...
#write.csv(gene_miRNA_df, file = "./3_results/miRNA_targets_september.csv")
...
barplot <- ggplot(gene_miRNA_df, aes(x = n_miRNAs)) +
  ...
  labs(
    ...
    x = "Number of DE miRNAs targeting the same gene",
    y = "Number of genes"
  ) +
  theme_minimal()
...
plot(barplot)
#ggsave(
#  filename = "./2_figures/miRNA_distribution_plot.png", # change to whatever you like
 # plot     = barplot,
#  width    = 8,    # in inches
  #height   = 6,    # in inches
 # dpi      = 300   # resolution
...
ggplot(down_miRNA_gene_targets, aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point(aes(color = log2FoldChange > 0), alpha = 1) +
  scale_color_manual(values = c("TRUE" = "#994455", "FALSE" = "grey")) +
  labs(title = "Targets of Downregulated miRNAs (Expected ↑)",
       x = "log2 Fold Change (mRNA)",
       y = "-log10 adjusted p-value") +
  theme_minimal()

ggplot(up_miRNA_gene_targets, aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point(aes(color = log2FoldChange < 0), alpha = 1) +
  scale_color_manual(values = c("TRUE" = "#0077BB", "FALSE" = "grey")) +
  labs(title = "Targets of Upregulated miRNAs (Expected ↓)",
       x = "log2 Fold Change (mRNA)",
       y = "-log10 adjusted p-value") +
  theme_minimal()
...
plot_miRNA_targets <- up_miRNA_gene_targets %>%
  filter(!is.na(sign_DE)) %>%
  ggplot(aes(x = log2FoldChange, y = -log10(padj), color = sign_DE, fill = sign_DE, alpha = sign_DE)) +
  ...
  labs(
    title = "Targets of Upregulated miRNAs (Expected ↓)",
    x = "Log2 Fold Change (mRNA)",
    y = "-Log10 adjusted p-value",
    color = "Effect in treated group",
    fill = "Effect in treated group",
    alpha = "Effect in treated group"
  ) +
  theme_bw() +
  theme(
...
plot_miRNA_targets2 <- down_miRNA_gene_targets %>%
  filter(!is.na(sign_DE)) %>%
  ggplot(aes(x = log2FoldChange, y = -log10(padj), color = sign_DE, fill = sign_DE, alpha = sign_DE)) +
  geom_point(size = 5, shape = 21, color = "black") +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", size = 0.4, col = "#3A3B3C") +
...
#write.csv(miRNA_target_numbers, file = "./3_results/sep_DE_mature_miRNA_hits_DE_genes_freq.csv")
